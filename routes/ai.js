...
// CONFIG
// Using "gpt-4o-mini" as the real model for "GPT-5 Mini" branding, unless user has custom proxy.
// If OPENAI_MODEL is set in env, use that.
...
router.post('/enrich', async (req, res) => {
    try {
        ...
        const memberRes = await db.query('SELECT * FROM crm.wa_members WHERE member_id = $1', [memberId]);
        if (memberRes.rows.length === 0) return res.status(404).json({ error: 'Member not found' });
        const member = memberRes.rows[0];

        const msgRes = await db.query('SELECT message_content FROM crm.wa_messages WHERE sender_id = $1 ORDER BY created_at DESC LIMIT 50', [memberId]);
        const messages = msgRes.rows.map(m => m.message_content).join('\n');

        if (!messages && !forceDeep) return res.json({ status: 'skipped', reason: 'No messages' });
        ...
        // SAVE TO DB
        // Mapping AI response to DB columns: role -> job_title, summary -> chat_profile_summary, score -> enrichment_priority
        await db.query(`
